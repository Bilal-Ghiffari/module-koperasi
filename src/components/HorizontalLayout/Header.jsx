...
// reactstrap
import { Row, Col, Dropdown, DropdownToggle, DropdownMenu, NavLink, NavItem } from 'reactstrap';
...
const Header = (props) => {
  ...
  const temp = JSON.parse(localStorage.getItem('resUser'));
  ...
  const tempModules =
    roles?.[0]?.module == null
      ? []
      : roles.map((item) => {
          const found = modules_constant.find((constItem) => constItem.id == item.module_code);
          if (found) {
            return {
              ...item,
              ...found,
      ...
      <header id="page-topbar">
        <div className="navbar-header py-2">
          <div className="d-flex ps-4">
            <a className="navbar-brand d-flex align-items-center column-gap-2" href="/">
              <img src={logoDarkCustom} alt="" height={45} className="d-inline-block align-text-top" />
          ...
          {listMenuUsed ? (
            <Nav tabs className="nav-tabs-custom nav-justified">
              {listMenuUsed.map((o, idx) => {
                const isActive = o.menu.some((p) => props.pathName.startsWith(p.href));
                return (
                  <NavItem key={idx} className="align-content-end pb-2">
                    <NavLink className={classnames({ active: isActive }) + 'cursor-pointer px-1'}>
              ...
              <NavItem className="align-content-end pb-2">
                <div className="rounded-circle header-profile-user cursor-pointer" onClick={() => navigate('/')}>
