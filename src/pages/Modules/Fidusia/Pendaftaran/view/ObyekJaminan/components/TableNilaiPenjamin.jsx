...
const TableNilaiPenjamin = ({
  ...
  const handleCheckboxChange = (row) => {
    const current = formik.values.object_jaminan.nilaiPenjaminanList || [];
    const exists = current.some((item) => item.id === row.id);
    const newSelected = exists
      ? current.filter((item) => item.id !== row.id)
      : [...current, row];

    formik.setFieldValue('object_jaminan.nilaiPenjaminanList', newSelected);
  };
      ...
      ? [
          {
            ...
            cell: (row) => (
              <Checkbox
                color="primary"
                checked={formik.values?.object_jaminan.nilaiPenjaminanList?.some(
                  (item) => item.id === row.id
                )}
...
export default TableNilaiPenjamin;
