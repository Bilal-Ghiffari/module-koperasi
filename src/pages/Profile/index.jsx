  ...
  return (
    <React.Fragment>
      <div className="container">
        <div className={`card shadow-sm rounded-3 border border-secondary border-opacity-25 hover-scale cursor-pointer ${item.iconImg ? 'py-2':'py-3'} `} style={{minHeight: '121px'}} onClick={() => handleClick()}>
          <div className="card-body d-flex align-items-center">
            <div className="me-3">

              {/* sebagian udah ada icons dari figma nya */}
              {item.iconImg ? (
                <div className="d-flex align-items-center justify-content-center rounded-3" style={{ height: '58px', width: '58px', backgroundColor: `rgba(${convertHexToRGB(item.color)}, 1)` }}>
                  <img src={item.iconImg} alt={`icons-${item.title}`} />
                </div>
              ) : (
                <div className="p-1 rounded-3" style={{ backgroundColor: `rgba(${convertHexToRGB(item.color)}, 0.25)` }}>
                  <i className={`bx ${item.icon} bx-md`} style={{ color: item.color }} />
                </div>
              )}
            </div>
            <div>
...
const ProfileLanding = () => {
  ...
  useEffect(() => {
    const tempModules = modules_constant.filter((o) => findPermission({ moduleCode: o.r_modules_code, isModule: true, action: 'permission' }));

    setModules(tempModules.map((o) => ({ ...o, link: o.url, title: o.module, text: 'Modul ' + o.module })));
  }, []);
            ...
            {modules.length ? (
              modules.map((item, idx) => (
                <Col sm={6} md={4} lg={3} key={'_card_' + idx}>
                  <CardCustom item={item} onClick={(e) => handleClicked(e, item)} />
                </Col>
              ))
            ) : (
              <div className="w-100 d-flex flex-column justify-content-center align-items-center" style={{ height: 'calc(100vh - 120px)' }}>
                <h3 className="text-secondary text-opacity-50 text-center">
                  Anda Tidak Punya Akses ke Modul Apapun <br /> Harap Hubungi Admin untuk Mendapat Akses Modul
                </h3>
                {/* <Button
                  color="success"
                  className="mt-4"
