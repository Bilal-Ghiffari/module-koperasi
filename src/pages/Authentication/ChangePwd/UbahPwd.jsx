...
import { Button, Card, CardBody, Form, FormFeedback, Input, InputGroup, Label, Row } from 'reactstrap';
...
export const UbahPwd = () => {
  ...
  const user = useMemo(() => {
    const ls = JSON.parse(localStorage.getItem('resUser'));
      ...
      <Card className="rounded-5">
        <CardBody className="container">
              ...
              <div className="col-md-10">
                <InputGroup>
                  <Input
                    ...
                    name="old_pass"
                    className={`form-control ${validation.touched.old_pass && validation.errors.old_pass ? 'is-invalid' : ''}`}
                    id="validationCustom01"
                    onChange={validation.handleChange}
                    onBlur={validation.handleBlur}
                    value={validation.values.old_pass}
                  />
                  <Button className="bg-white text-black border-start-0 border-secondary border-opacity-25" onClick={() => tglVisiblePwd('old_pass')}>
                    {showPassword.old_pass ? <FaEyeSlash size={20} /> : <FaEye size={20} />}
                  </Button>
                </InputGroup>
              ...
              <div className="col-md-10">
                <InputGroup>
                  <Input
                    ...
                    name="new_pass"
                    className={`form-control ${validation.touched.new_pass && validation.errors.new_pass ? 'is-invalid' : ''}`}
                    id="validationCustom01"
                    onChange={validation.handleChange}
                    onBlur={validation.handleBlur}
                    value={validation.values.new_pass}
                  />
                  <Button className="bg-white text-black border-start-0 border-secondary border-opacity-25" onClick={() => tglVisiblePwd('new_pass')}>
                    {showPassword.new_pass ? <FaEyeSlash size={20} /> : <FaEye size={20} />}
                  </Button>
                </InputGroup>
              ...
              <div className="col-md-10">
                <InputGroup>
                  <Input
                    ...
                    name="repassword"
                    className={`form-control ${validation.touched.repassword && validation.errors.repassword ? 'is-invalid' : ''}`}
                    id="validationCustom01"
                    onChange={validation.handleChange}
                    onBlur={validation.handleBlur}
                    value={validation.values.repassword}
                  />
                  <Button className="bg-white text-black border-start-0 border-secondary border-opacity-25" onClick={() => tglVisiblePwd('repassword')}>
                    {showPassword.repassword ? <FaEyeSlash size={20} /> : <FaEye size={20} />}
                  </Button>
                </InputGroup>
                ...
                {validation.touched.repassword && validation.values.new_pass != validation.values.repassword ? (
                  <FormFeedback className="d-block" type="invalid">
          ...
          <div className="actions clearfix justify-content-end d-flex">
            <button onClick={validation.handleSubmit} className="btn bg-primary text-white">
              Simpan
            </button>
          </div>
        </CardBody>
